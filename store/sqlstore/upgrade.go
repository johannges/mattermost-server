...
func UpgradeDatabaseToVersion55(sqlStore SqlStore) {
	...
	sqlStore.AlterColumnTypeIfExists("Groups", "Props", "text", "text")
	...
	if sqlStore.DriverName() == model.DATABASE_DRIVER_POSTGRES {
		if _, err := transaction.Exec("ALTER TABLE Groups ALTER COLUMN Props SET DATA TYPE jsonb USING Props::jsonb"); err != nil {
			failureF(err.Error())
		}
	}
